package club.doki7.stb.vorbis.handle;
...
/// Represents an opaque handle type {@code stb_vorbis}.
///
/// ## Contracts
///
...
@ValueBasedCandidate
@UnsafeConstructor
public record STB_Vorbis(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code stb_vorbis} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<STB_Vorbis> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable STB_Vorbis read() {
            MemorySegment s = readRaw();
            if (s.equals(MemorySegment.NULL)) {
                return null;
            }
            return new STB_Vorbis(s);
        }

        public void write(@Nullable STB_Vorbis value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable STB_Vorbis read(long index) {
        ...
        public void write(long index, @Nullable STB_Vorbis value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable STB_Vorbis[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable STB_Vorbis value0, @Nullable STB_Vorbis ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable STB_Vorbis @NotNull [] values) {
        ...
        public static Ptr allocate(Arena arena, @NotNull Collection<@Nullable STB_Vorbis> values) {
            Ptr ret = allocate(arena, values.size());
            int i = 0;
            for (@Nullable STB_Vorbis value : values) {
                ret.write(i, value);
                i += 1;
            }
            return ret;
        ...
        public static Ptr allocateV(Arena arena, @Nullable STB_Vorbis value0, @Nullable STB_Vorbis ...values) {
        ...
        @Override
        public @NotNull Iterator<STB_Vorbis> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<STB_Vorbis> {
            ...
            @Override
            public @Nullable STB_Vorbis next() {
                if (!hasNext()) {
